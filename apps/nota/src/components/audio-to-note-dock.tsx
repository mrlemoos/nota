  ...
  const runPipeline = useCallback(
    async (blob: Blob, mime: string, targetNoteId: string, uid: string) => {
      ...
      try {
        ...
        const msg =
          e instanceof Error ? e.message : 'Could not generate study notes';
        setError(msg);
      }
    },
    [
  ...
  const togglePauseResume = useCallback(() => {
    ...
    if (rec.state === 'recording') {
      elapsedActiveMsRef.current +=
        performance.now() - activeSegmentStartRef.current;
      try {
        rec.pause();
      } catch {
        return;
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {pauseSupported ? (
              <NotaButton
                ...
                aria-label={
                  recorderPaused ? 'Resume recording' : 'Pause recording'
                }
              >
